...
Mesh::Mesh(Vertex vert[], int vertC, //vertex
	unsigned int index[], int indiceCount, //index
	ID3D11Device* device)
{
	vertCount = vertC;
	indCount = indiceCount;
	vertices = vert;
	indices = index;
	d = device;
	Init();
...
Mesh::Mesh(char * filename, ID3D11Device* device)
{
	...
	std::vector<UINT> indi;           // Indices of these verts
	...
	while (obj.good())
	{
		...
		else if (chars[0] == 'f')
		{
			...
			indi.push_back(vertCounter); vertCounter += 1;
			indi.push_back(vertCounter); vertCounter += 1;
			indi.push_back(vertCounter); vertCounter += 1;
...
void Mesh::Init()
{
	...
	d->CreateBuffer(&vbd, &initialVertexData, &vertexBuffer);
	...
	ibd.ByteWidth = sizeof(int) * indCount;         // 3 = number of indices in the buffer
	...
	// Actually create the buffer with the initial data
	// - Once we do this, we'll NEVER CHANGE THE BUFFER AGAIN
	d->CreateBuffer(&ibd, &initialIndexData, &indexBuffer);
